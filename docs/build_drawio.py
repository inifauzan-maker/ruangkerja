...
class Diagram:
    def __init__(self):
        self.file = ET.Element("mxfile", {"host":"app.diagrams.net","modified":"2026-09-04T00:00:00.000Z","agent":"Codex","version":"24.7.17","type":"device"})
...
r=d.page("sitemap-v2","01 - Sitemap"); d.title(r,"t","SITEMAP RUANGKERJA / TIMMANAGER","Publik, workspace, kolaborasi, file, integrasi, dan analitik")
nodes=[("visitor","Pengunjung",70,180),("login","Login",70,300),("register","Registrasi",70,390),("user","Pengguna Terautentikasi",380,180),("home","Beranda Tim & Proyek",360,300),("search","Pencarian Global",360,400),("reports","Dashboard Laporan",360,500),("profile","Profil & WhatsApp",360,600),("team","Tim, Anggota & Undangan",700,300),("board","Papan Proyek",700,420),("kanban","Kanban & Filter",1040,230),("summary","Ringkasan",1040,330),("chat","Chat Grup",1040,430),("announce","Pengumuman",1040,530),("task","Detail Tugas",1040,650),("file","Preview & Versi File",1340,650)]
...
for i,(s,t) in enumerate([("visitor","login"),("visitor","register"),("login","user"),("user","home"),("user","search"),("user","reports"),("user","profile"),("user","team"),("user","board"),("board","kanban"),("board","summary"),("board","chat"),("board","announce"),("board","task"),("task","file")]):d.edge(r,f"e{i}",s,t)

r=d.page("erd-core-v2","02 - ERD Core"); d.title(r,"t","ERD CORE","Identitas, tim, membership, proyek, list Kanban, dan tugas")
entities=[("users","USERS<br><b>PK id</b><br>name, email, password<br>profile & avatar",60,190),("teams","TEAMS<br><b>PK id</b><br><b>FK owner_id</b><br>name, slug, description",370,190),("members","TEAM_USER<br><b>FK team_id, user_id</b><br>role",220,510),("invites","TEAM_INVITATIONS<br>team_id, inviter_id<br>email, role, token_hash<br>expires_at, accepted_at",560,510),("boards","BOARDS<br><b>FK team_id</b><br>name, description<br>download_permission",700,190),("lists","BOARD_LISTS<br><b>FK board_id</b><br>title, color, position",1020,190),("tasks","TASKS<br>board_list_id, creator_id<br>title, description, priority<br>due_at, position",1300,190)]
for cid,label,x,y in entities:d.box(r,cid,label,x,y,230,175,"FFFFFF",GREEN,INK,False,12)
for i,(s,t,l) in enumerate([("users","teams","owner"),("users","members","1:N"),("teams","members","1:N"),("teams","invites","1:N"),("users","invites","inviter"),("teams","boards","1:N"),("boards","lists","1:N"),("lists","tasks","1:N"),("users","tasks","creator")]):d.edge(r,f"e{i}",s,t,l)

r=d.page("erd-collaboration-v2","03 - ERD Collaboration & File"); d.title(r,"t","ERD KOLABORASI, FILE, DAN NOTIFIKASI","Multi-assignee, checklist, komentar, activity, file berversi, Gmail, dan WhatsApp")
items=[("task","TASKS",70,180,180,70),("tu","TASK_USER<br>task_id, user_id",320,160,220,95),("check","TASK_CHECKLIST_ITEMS<br>task_id, creator_id<br>title, is_completed<br>completed_at, position",320,310,250,140),("comment","TASK_COMMENTS<br>task_id, user_id, body",660,160,230,95),("mention","TASK_COMMENT_MENTIONS<br>comment_id, user_id",660,330,250,95),("activity","TASK_ACTIVITIES<br>task_id, actor_id<br>type, metadata JSON",1000,160,240,115),("attach","ATTACHMENTS<br>polymorphic attachable<br>root_attachment_id, version<br>is_current, scan_status",1000,350,270,145),("user","USERS",1330,180,180,70),("wa","WHATSAPP_CONNECTIONS<br>user_id, token terenkripsi<br>preferences, quiet hours",80,650,270,130),("walog","WHATSAPP_NOTIFICATION_LOGS<br>connection_id, task_id<br>event, status, schedule/error",450,650,290,140),("email","EMAIL_NOTIFICATION_LOGS<br>team_id, sender_id, recipient<br>event, status, send/error",850,650,290,140)]
...
r=d.page("rbac-v2","04 - RBAC"); d.title(r,"t","ROLE-BASED ACCESS CONTROL","Role per tim dan proteksi resource dengan membership checks")
...
r=d.page("notifications-v2","08 - Notification & Reports"); d.title(r,"t","NOTIFIKASI DAN LAPORAN","Gmail SMTP, WhatsApp Cloud API, queue/log, agregasi dashboard, PDF, dan XLSX")
blocks=[("events","EVENT<br>Tugas • Chat • Pengumuman<br>Reminder tenggat",60,190,280,140),("gmail","Gmail SMTP<br>Undangan tim",470,170,240,100),("email","Email Logs<br>sent / failed",470,350,240,90),("prefs","Preferensi + quiet hours",820,170,260,90),("queue","Queue job<br>idempotency & retry",820,340,260,110),("api","WhatsApp Cloud API",1200,340,260,80),("walog","WA Logs<br>pending/sent/failed/skipped",1200,530,270,100),("agg","Report Aggregator<br>status • workload • activity • logs",450,660,320,130),("dash","Dashboard<br>filter proyek & periode",850,680,280,90),("export","PDF + XLSX",1260,680,210,75)]
...
r=d.page("guide-v2","09 - User Guide"); d.title(r,"t","PANDUAN PENGGUNA","Perjalanan utama dari registrasi hingga ekspor laporan")
steps=[("1","Registrasi / Login","Buat akun dan masuk"),("2","Lengkapi Profil","Profil, avatar, password"),("3","Buat / Gabung Tim","Undangan Gmail dan role"),("4","Buat Proyek","Owner/Admin membuat papan"),("5","Buat Tugas","Assignee, tenggat, file"),("6","Berkolaborasi","Checklist, komentar, chat"),("7","Hubungkan WhatsApp","Token, preferensi, quiet hours"),("8","Cari & Pantau","Search, filter, ringkasan"),("9","Buka Laporan","Workload, activity, PDF/XLSX")]
...
r=d.page("deployment-v2","10 - Deployment"); d.title(r,"t","DEPLOYMENT DAN OPERASI","Runtime, rahasia .env, queue, scheduler, storage, integrasi, dan quality gate")
blocks=[("browser","Browser",60,210,180,60),("web","Laravel Web\nBlade + Controller",340,190,260,100),("db","MySQL",740,160,220,70),("storage","Private Storage",740,290,220,80),("queue","Queue Worker",340,480,260,70),("schedule","Scheduler\nDue reminder",340,630,260,80),("gmail","smtp.gmail.com",740,500,220,70),("meta","WhatsApp Graph API",740,650,220,70),("env","RAHASIA .ENV<br>APP_KEY • DB • Gmail<br>ClamAV config<br>WA token terenkripsi",1100,180,360,190),("quality","QUALITY GATE<br>migrate:status<br>92 tests / 358 assertions<br>view:cache • npm run build<br>Pint",1100,520,360,180)]
for cid,label,x,y,w,h in blocks:d.box(r,cid,label,x,y,w,h,"FFFFFF",GREEN,INK,False,14)
for i,(s,t,l) in enumerate([("browser","web","HTTPS"),("web","db","Eloquent"),("web","storage","authorized I/O"),("web","queue","dispatch"),("schedule","queue","enqueue"),("queue","gmail","SMTP"),("queue","meta","Graph API"),("env","web","config"),("env","queue","config")]):d.edge(r,f"e{i}",s,t,l)
