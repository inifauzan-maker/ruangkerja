<x-layout title="Undangan Tim">
    <div class="grid min-h-dvh place-items-center bg-[#f1f2f0] px-4 py-10">
        <main class="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-xl sm:p-9">
            <span class="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-[#F5C542] text-xl font-extrabold text-[#123A70]">{{ str($teamInvitation->team->name)->substr(0, 2)->upper() }}</span>
            ...
            <form method="POST" action="{{ route('team-invitations.accept', [$teamInvitation, $token]) }}" class="mt-7">@csrf<button class="w-full rounded-xl bg-[#123A70] px-5 py-3.5 font-bold text-white">Terima dan bergabung</button></form>
