    ...
    <div class="min-h-dvh bg-[#f1f2f0]">
        ...
        <main class="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[20rem_minmax(0,1fr)] lg:px-8 lg:py-8">
            <aside class="grid content-start gap-6">
                <section class="overflow-hidden rounded-3xl bg-[#153d36] text-white shadow-xl">
                    <div class="bg-[radial-gradient(circle_at_top_right,rgba(242,184,75,.35),transparent_45%)] p-6 text-center">
                        ...
                        <p class="mt-1 text-sm text-emerald-100/60">{{ $user->job_title ?: 'Anggota RuangKerja' }}</p>
                        <p class="mt-3 break-all text-xs text-emerald-100/45">{{ $user->email }}</p>
                    </div>
                    <div class="grid grid-cols-3 border-t border-white/10 text-center">
                        <div class="p-4"><p class="text-xl font-extrabold text-[#f2b84b]">{{ $user->teams_count + $user->joined_teams_count }}</p><p class="mt-1 text-[10px] uppercase tracking-wider text-white/45">Tim</p></div>
                        <div class="border-x border-white/10 p-4"><p class="text-xl font-extrabold text-[#f2b84b]">{{ $projectsCount }}</p><p class="mt-1 text-[10px] uppercase tracking-wider text-white/45">Proyek</p></div>
                        <div class="p-4"><p class="text-xl font-extrabold text-[#f2b84b]">{{ $user->tasks_count }}</p><p class="mt-1 text-[10px] uppercase tracking-wider text-white/45">Tugas</p></div>
                    </div>
                </section>
                ...
                <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <form method="POST" action="{{ route('profile.avatar.update') }}" enctype="multipart/form-data" class="mt-4 grid gap-3">@csrf
                        <input type="file" name="avatar" required accept=".jpg,.jpeg,.png,.webp" class="min-w-0 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-3 text-xs file:mr-2 file:rounded-lg file:border-0 file:bg-emerald-50 file:px-3 file:py-2 file:font-bold file:text-emerald-700">
                        <p class="text-[11px] leading-5 text-slate-400">JPG, PNG, atau WebP. Maksimal 2 MB.</p>
                        <button class="rounded-xl bg-[#f2b84b] px-4 py-2.5 text-sm font-extrabold text-[#153d36]">Upload foto</button>
                    </form>
                    @if ($user->avatar_path)
            ...
            <div class="grid min-w-0 content-start gap-6">
                <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" class="mt-7 grid gap-5">@csrf @method('PATCH')
                        <div class="grid gap-5 sm:grid-cols-2">
                            <label class="grid gap-2 text-sm font-bold">Nama lengkap<input name="name" value="{{ old('name', $user->name) }}" required maxlength="100" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100"></label>
                            <label class="grid gap-2 text-sm font-bold">Email<input name="email" type="email" value="{{ old('email', $user->email) }}" required maxlength="255" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100"></label>
                            <label class="grid gap-2 text-sm font-bold">Jabatan <span class="font-normal text-slate-400">(opsional)</span><input name="job_title" value="{{ old('job_title', $user->job_title) }}" maxlength="100" placeholder="Contoh: Product Designer" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100"></label>
                            <label class="grid gap-2 text-sm font-bold">Nomor telepon <span class="font-normal text-slate-400">(opsional)</span><input name="phone" value="{{ old('phone', $user->phone) }}" maxlength="30" placeholder="+62 812 3456 7890" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100"></label>
                        </div>
                        <label class="grid gap-2 text-sm font-bold">Bio <span class="font-normal text-slate-400">(opsional)</span><textarea name="bio" rows="4" maxlength="1000" placeholder="Ceritakan sedikit tentang pekerjaan dan fokus Anda..." class="resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100">{{ old('bio', $user->bio) }}</textarea></label>
                        <button class="w-fit rounded-xl bg-[#153d36] px-5 py-3 text-sm font-bold text-white">Simpan profil</button>
                    </form>
                </section>
                ...
                <section class="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-5 shadow-sm sm:p-7">
                    <div class="flex flex-col gap-5 sm:flex-row sm:items-center">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            @endif
                            <a href="{{ route('profile.whatsapp.show') }}" class="rounded-xl bg-[#153d36] px-5 py-3 text-sm font-bold text-white">Kelola koneksi</a>
                        </div>
                    </div>
                </section>
                ...
                <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
                    ...
                    <form method="POST" action="{{ route('profile.password.update') }}" class="mt-7 grid gap-5">@csrf @method('PATCH')
                        <label class="grid gap-2 text-sm font-bold">Password saat ini<input name="current_password" type="password" required autocomplete="current-password" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100"></label>
                        <div class="grid gap-5 sm:grid-cols-2"><label class="grid gap-2 text-sm font-bold">Password baru<input name="password" type="password" required autocomplete="new-password" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100"></label><label class="grid gap-2 text-sm font-bold">Konfirmasi password<input name="password_confirmation" type="password" required autocomplete="new-password" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100"></label></div>
                        <button class="w-fit rounded-xl bg-[#153d36] px-5 py-3 text-sm font-bold text-white">Perbarui password</button>
                    </form>
                </section>
