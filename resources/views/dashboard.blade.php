<x-layout title="Beranda">
    ...
    <div class="flex h-dvh flex-col overflow-hidden bg-[#f1f2f0]">
        <header class="shrink-0 border-b border-slate-200 bg-white">
            <div class="flex h-16 items-center gap-4 px-4 sm:px-6 lg:px-8">
                <a href="{{ route('dashboard') }}" class="flex min-w-0 items-center gap-3">
                    <span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#F5C542] text-xs font-extrabold text-[#123A70]">VM</span>
                    <div class="hidden sm:block"><p class="font-extrabold tracking-tight">Ruang Kerja _ Villa Merah</p><p class="text-[10px] font-bold uppercase tracking-[.16em] text-slate-400">Beranda</p></div>
                </a>
                <form method="GET" action="{{ route('search') }}" class="mx-auto hidden w-full max-w-md md:block">
                    <label class="relative block"><svg class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="11" cy="11" r="7" stroke-width="2"/><path d="m20 20-3.5-3.5" stroke-width="2"/></svg><input id="dashboard-search" name="q" type="search" required maxlength="100" placeholder="Cari tim, proyek, tugas, file..." class="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-blue-600 focus:bg-white focus:ring-4 focus:ring-blue-100"></label>
                </form>
                <div class="ml-auto flex items-center gap-3">
                    ...
                    @endif
                    <a href="{{ route('reports.index') }}" class="rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold text-slate-600 hover:bg-blue-50 hover:text-blue-700">Laporan</a>
        ...
        @if (session('status'))
            <div data-toast class="fixed right-5 top-5 z-[70] flex items-center gap-3 rounded-xl bg-[#123A70] px-4 py-3 text-sm font-semibold text-white shadow-xl"><span class="grid h-6 w-6 place-items-center rounded-full bg-emerald-400 text-[#123A70]">✓</span>{{ session('status') }}</div>
        @endif
            ...
            <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                <section>
                    <div class="mb-5 flex items-center gap-3 border-b border-slate-300 pb-4">
                        ...
                        <button data-open-dialog="team-dialog" class="ml-auto grid h-11 w-11 place-items-center rounded-full bg-[#F5C542] text-2xl font-bold text-[#123A70] shadow-md hover:scale-105 hover:bg-amber-400" aria-label="Buat tim">+</button>
                    </div>
                ...
                <section class="mt-10">
                    <div class="mb-5 flex items-center gap-3 border-b border-slate-300 pb-4">
                        ...
                        <button data-open-dialog="project-dialog" {{ $managedTeams->isEmpty() ? 'disabled' : '' }} class="ml-auto grid h-11 w-11 place-items-center rounded-full bg-[#F5C542] text-2xl font-bold text-[#123A70] shadow-md hover:scale-105 hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Buat proyek">+</button>
                    </div>
    ...
    <dialog id="team-dialog" class="m-auto w-[calc(100%-2rem)] max-w-md rounded-3xl bg-white p-0 shadow-2xl backdrop:bg-slate-950/50 backdrop:backdrop-blur-sm"><form method="POST" action="{{ route('teams.store') }}" class="p-6">@csrf<div class="flex items-start justify-between"><div><p class="text-xs font-bold uppercase tracking-wider text-emerald-700">Tim baru</p><h2 class="mt-2 text-2xl font-extrabold">Buat ruang kolaborasi</h2></div><button type="button" data-close-dialog class="grid h-9 w-9 place-items-center rounded-full bg-slate-100 text-xl">×</button></div><div class="mt-6 grid gap-4"><label class="grid gap-2 text-sm font-bold">Nama tim<input name="name" required maxlength="100" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" placeholder="Contoh: Tim Produk"></label><label class="grid gap-2 text-sm font-bold">Deskripsi<textarea name="description" maxlength="1000" rows="3" class="resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" placeholder="Apa fokus tim ini?"></textarea></label><button class="rounded-xl bg-[#123A70] px-5 py-3 font-bold text-white">Buat tim</button></div></form></dialog>

    <dialog id="project-dialog" class="m-auto w-[calc(100%-2rem)] max-w-md rounded-3xl bg-white p-0 shadow-2xl backdrop:bg-slate-950/50 backdrop:backdrop-blur-sm"><form method="POST" action="{{ route('projects.store') }}" class="p-6">@csrf<div class="flex items-start justify-between"><div><p class="text-xs font-bold uppercase tracking-wider text-emerald-700">Proyek baru</p><h2 class="mt-2 text-2xl font-extrabold">Mulai papan kerja</h2></div><button type="button" data-close-dialog class="grid h-9 w-9 place-items-center rounded-full bg-slate-100 text-xl">×</button></div><div class="mt-6 grid gap-4"><label class="grid gap-2 text-sm font-bold">Tim<select name="team_id" required class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100">@foreach ($ownedTeams as $team)<option value="{{ $team->id }}">{{ $team->name }}</option>@endforeach</select></label><label class="grid gap-2 text-sm font-bold">Nama proyek<input name="name" required maxlength="120" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" placeholder="Contoh: Website baru"></label><label class="grid gap-2 text-sm font-bold">Deskripsi<textarea name="description" maxlength="1000" rows="3" class="resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" placeholder="Tujuan singkat proyek..."></textarea></label><button class="rounded-xl bg-[#123A70] px-5 py-3 font-bold text-white">Buat proyek</button></div></form></dialog>

    <dialog id="member-dialog" class="m-auto w-[calc(100%-2rem)] max-w-md rounded-3xl bg-white p-0 shadow-2xl backdrop:bg-slate-950/50 backdrop:backdrop-blur-sm"><form id="member-form" method="POST" data-action-template="{{ url('/teams/__TEAM__/members') }}" class="p-6">@csrf<div class="flex items-start justify-between"><div><p class="text-xs font-bold uppercase tracking-wider text-emerald-700">Anggota tim</p><h2 class="mt-2 text-2xl font-extrabold">Tambahkan rekan kerja</h2></div><button type="button" data-close-dialog class="grid h-9 w-9 place-items-center rounded-full bg-slate-100 text-xl">×</button></div><div class="mt-6 grid gap-4"><label class="grid gap-2 text-sm font-bold">Pilih tim<select id="member-team-select" required class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100">@foreach ($ownedTeams as $team)<option value="{{ $team->id }}">{{ $team->name }}</option>@endforeach</select></label><label class="grid gap-2 text-sm font-bold">Email pengguna<input name="email" type="email" required class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" placeholder="rekan@email.com"></label><p class="text-xs leading-5 text-slate-400">Pengguna harus sudah memiliki akun Ruang Kerja _ Villa Merah.</p><button class="rounded-xl bg-[#123A70] px-5 py-3 font-bold text-white">Tambahkan anggota</button></div></form></dialog>
