<x-layout :title="$board->name">
    ...
    <div class="flex h-dvh overflow-hidden bg-[#f4f5f3]" data-board>
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col bg-[#153d36] text-white transition-transform duration-300 lg:static lg:w-56 lg:translate-x-0 xl:w-60">
            <a href="{{ route('dashboard') }}" class="flex h-20 items-center gap-3 border-b border-white/10 px-6 hover:bg-white/5">
                <span class="grid h-10 w-10 place-items-center rounded-xl bg-[#f2b84b] font-extrabold text-[#153d36]">R</span>
                <div>
                    <p class="font-extrabold tracking-tight">RuangKerja</p>
                    <p class="text-xs text-emerald-100/50">Team workspace</p>
                </div>
            </a>

            <nav class="flex-1 space-y-1 px-3 py-6 text-sm font-semibold">
                <a href="{{ route('boards.show', $board) }}" class="flex items-center gap-3 rounded-xl px-3 py-3 {{ ($activeSection ?? 'kanban') === 'kanban' ? 'bg-white/10 text-white' : 'text-emerald-50/60 hover:bg-white/5 hover:text-white' }}">
                ...
                <a href="{{ route('boards.chat', $board) }}" class="flex w-full items-center gap-3 rounded-xl px-3 py-3 {{ ($activeSection ?? 'kanban') === 'chat' ? 'bg-white/10 text-white' : 'text-emerald-50/60 hover:bg-white/5 hover:text-white' }}">
                ...
                <a href="{{ route('boards.announcements', $board) }}" class="flex w-full items-center gap-3 rounded-xl px-3 py-3 {{ ($activeSection ?? 'kanban') === 'announcements' ? 'bg-white/10 text-white' : 'text-emerald-50/60 hover:bg-white/5 hover:text-white' }}">
                ...
                <p class="px-3 pb-2 pt-8 text-[11px] font-bold uppercase tracking-[.18em] text-emerald-100/35">Papan saya</p>
                @foreach ($boards as $availableBoard)
                    <a href="{{ route('boards.show', $availableBoard) }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 {{ $availableBoard->is($board) ? 'bg-[#f2b84b] text-[#153d36]' : 'text-emerald-50/65 hover:bg-white/5 hover:text-white' }}">
                        <span class="h-2.5 w-2.5 rounded-full {{ $availableBoard->is($board) ? 'bg-[#153d36]' : 'bg-emerald-300/40' }}"></span>
            ...
            <div class="border-t border-white/10 p-4">
                <div class="flex items-center gap-3 rounded-xl bg-white/5 p-3">
                    <a href="{{ route('profile.show') }}" title="Profil saya"><x-current-user-avatar class="h-9 w-9 border-2 border-[#f2b84b] text-xs" /></a>
                    <div class="min-w-0 flex-1">
                        <p class="truncate text-sm font-bold">{{ auth()->user()->name }}</p>
                        <p class="truncate text-xs text-emerald-50/45">{{ auth()->user()->email }}</p>
                    </div>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button title="Keluar" class="rounded-lg p-2 text-emerald-50/50 hover:bg-white/10 hover:text-white">
        ...
        <main class="flex h-dvh min-w-0 flex-1 flex-col overflow-hidden">
            <header class="shrink-0 border-b border-slate-200 bg-white">
                <div class="flex h-20 items-center gap-4 px-4 sm:px-6 lg:px-8">
                    ...
                    <form method="GET" action="{{ route('search') }}" class="relative hidden w-64 md:block">
                        ...
                        <input name="q" type="search" required maxlength="100" placeholder="Cari semua..." class="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100">
                    </form>
                    ...
                    <a href="{{ route('profile.show') }}" title="Profil saya"><x-current-user-avatar class="h-10 w-10 border-2 border-[#f2b84b] text-xs" /></a>
                </div>
            ...
            @if (session('status'))
                <div data-toast class="fixed right-5 top-5 z-[70] flex items-center gap-3 rounded-xl bg-[#153d36] px-4 py-3 text-sm font-semibold text-white shadow-xl">
                    <span class="grid h-6 w-6 place-items-center rounded-full bg-emerald-400 text-[#153d36]">✓</span>{{ session('status') }}
                </div>
            @endif
            @if ($errors->any())
            ...
            @if (($activeSection ?? 'kanban') === 'kanban')
                ...
                <div class="mb-4 flex shrink-0 flex-wrap items-center justify-between gap-3">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <form method="POST" action="{{ route('boards.lists.store', $board) }}" class="flex gap-2">
                            @csrf
                            <input name="title" required maxlength="80" placeholder="List baru..." aria-label="Nama list baru" class="w-32 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:w-44 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100 sm:w-40">
                        ...
                        <button data-open-task-modal data-list-id="{{ $board->lists->first()?->id }}" class="inline-flex items-center gap-2 rounded-xl bg-[#153d36] px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-[#205148]">
                ...
                <form method="GET" action="{{ route('boards.show', $board) }}" class="mb-3 grid shrink-0 gap-2 rounded-2xl border border-slate-200 bg-white p-3 sm:grid-cols-2 lg:grid-cols-[minmax(180px,1fr)_repeat(3,minmax(130px,auto))_auto_auto]">
                    <input type="search" name="q" value="{{ $filters['q'] ?? '' }}" maxlength="100" placeholder="Cari judul/deskripsi..." class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-emerald-600">
                    ...
                    <button class="rounded-xl bg-[#153d36] px-4 py-2 text-sm font-bold text-white">Filter</button>
                                    ...
                                    <div draggable="true" data-task-card data-task-id="{{ $task->id }}" data-update-url="{{ route('boards.tasks.update', [$board, $task]) }}" class="group cursor-grab rounded-xl border border-slate-200 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md active:cursor-grabbing">
                                        <div class="flex items-start gap-3">
                                            <div class="min-w-0 flex-1">
                                                ...
                                                <h3 class="task-title text-sm font-extrabold leading-5 text-slate-800"><a href="{{ route('boards.tasks.show', [$board, $task]) }}" class="hover:text-emerald-700">{{ $task->title }}</a></h3>
                                                <div class="mt-2 flex items-center justify-between gap-2">
                                                    <div class="flex -space-x-1.5">@forelse ($task->assignees->take(4) as $assignee)<span title="{{ $assignee->name }}" class="grid h-6 w-6 place-items-center rounded-full border-2 border-white bg-[#153d36] text-[8px] font-extrabold text-white">{{ str($assignee->name)->substr(0, 2)->upper() }}</span>@empty<span class="text-[10px] font-semibold text-slate-400">Belum ada assignee</span>@endforelse</div>
                                                ...
                                                <details class="mt-2"><summary class="cursor-pointer list-none text-[10px] font-bold text-emerald-700 hover:text-emerald-900">+ Tambah lampiran</summary><form method="POST" action="{{ route('boards.tasks.attachments.store', [$board, $task]) }}" enctype="multipart/form-data" class="mt-2 grid gap-2 rounded-lg bg-slate-50 p-2">@csrf<input type="file" name="attachments[]" multiple required accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.jpg,.jpeg,.png,.webp,.zip" class="min-w-0 text-[10px] file:mr-2 file:rounded file:border-0 file:bg-white file:px-2 file:py-1 file:font-bold"><button class="rounded-md bg-[#153d36] px-2 py-1.5 text-[10px] font-bold text-white">Upload</button></form></details>
                                            </div>
                                        ...
                                        <div class="mt-3 flex items-center gap-2 border-t border-slate-100 pt-2.5">
                                            <div class="grid h-7 w-7 place-items-center rounded-full bg-[#153d36] text-[10px] font-extrabold text-white">{{ str($task->creator->name)->substr(0, 2)->upper() }}</div>
            ...
            @elseif ($activeSection === 'summary')
                ...
                <section class="min-h-0 flex-1 overflow-y-auto px-4 py-6 sm:px-6 lg:px-8">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        ...
                        <div class="rounded-2xl bg-[#153d36] p-5 text-white shadow-sm"><p class="text-xs font-bold uppercase tracking-wider text-emerald-100/50">Progress</p><p class="mt-3 text-3xl font-extrabold">{{ $progress }}%</p><div class="mt-4 h-2 rounded-full bg-white/10"><div class="h-2 rounded-full bg-[#f2b84b]" style="width: {{ $progress }}%"></div></div></div>
                    </div>
                ...
                <section class="mx-auto flex min-h-0 w-full max-w-5xl flex-1 flex-col px-4 py-6 sm:px-6 lg:px-8">
                    ...
                    <div class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                        <div id="chat-messages" class="flex-1 space-y-5 overflow-y-auto p-5 sm:p-7">
                            @forelse ($board->messages->reverse() as $message)
                                <div class="flex gap-3 {{ $message->user_id === auth()->id() ? 'flex-row-reverse' : '' }}">
                                    <div class="grid h-9 w-9 shrink-0 place-items-center rounded-full {{ $message->user_id === auth()->id() ? 'bg-[#f2b84b] text-[#153d36]' : 'bg-[#153d36] text-white' }} text-xs font-extrabold">{{ str($message->user->name)->substr(0, 2)->upper() }}</div>
                                    <div class="max-w-[78%] {{ $message->user_id === auth()->id() ? 'text-right' : '' }}"><div class="mb-1 flex items-center gap-2 {{ $message->user_id === auth()->id() ? 'justify-end' : '' }}"><span class="text-xs font-bold">{{ $message->user->name }}</span><span class="text-[10px] text-slate-400">{{ $message->created_at->format('H:i') }}</span></div>@if ($message->body !== '')<p class="whitespace-pre-line rounded-2xl px-4 py-3 text-left text-sm leading-6 {{ $message->user_id === auth()->id() ? 'rounded-tr-sm bg-[#153d36] text-white' : 'rounded-tl-sm bg-slate-100 text-slate-700' }}">{{ $message->body }}</p>@endif<x-attachments :attachments="$message->attachments" class="{{ $message->user_id === auth()->id() ? 'justify-end' : '' }}" /></div>
                                </div>
                            @empty
                        ...
                        <form method="POST" action="{{ route('boards.messages.store', $board) }}" enctype="multipart/form-data" class="flex gap-2 border-t border-slate-100 p-4 sm:gap-3">
                            @csrf
                            <label class="grid h-11 w-11 shrink-0 cursor-pointer place-items-center rounded-xl border border-slate-200 bg-slate-50 text-lg hover:bg-emerald-50" title="Pilih lampiran">📎<input type="file" name="attachments[]" multiple class="sr-only" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.jpg,.jpeg,.png,.webp,.zip"></label>
                            <textarea name="body" maxlength="2000" rows="1" placeholder="Tulis pesan atau kirim file..." class="min-h-11 flex-1 resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100"></textarea>
                            <button class="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#153d36] text-white hover:bg-[#205148]" aria-label="Kirim pesan"><svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12 3 4l18 8-18 8 2-8Zm0 0h9"/></svg></button>
                        </form>
                    </div>
                </section>
                ...
                <section class="mx-auto min-h-0 w-full max-w-6xl flex-1 overflow-y-auto px-4 py-6 sm:px-6 lg:px-8">
                    <div class="grid gap-6 lg:grid-cols-[.7fr_1.3fr]">
                        <form method="POST" action="{{ route('boards.announcements.store', $board) }}" enctype="multipart/form-data" class="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            ...
                            <div class="mt-5 grid gap-4"><label class="grid gap-2 text-sm font-bold">Judul<input name="title" required maxlength="150" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100" placeholder="Contoh: Jadwal rilis berubah"></label><label class="grid gap-2 text-sm font-bold">Isi<textarea name="body" required maxlength="5000" rows="5" class="resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100" placeholder="Tulis detail pengumuman..."></textarea></label><label class="grid gap-2 text-sm font-bold">Lampiran <span class="font-normal text-slate-400">maks. 5 file, masing-masing 10 MB</span><input type="file" name="attachments[]" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.jpg,.jpeg,.png,.webp,.zip" class="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-3 text-xs file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-50 file:px-3 file:py-2 file:font-bold file:text-emerald-700"></label><label class="flex items-center gap-3 text-sm text-slate-600"><input type="checkbox" name="is_pinned" value="1" class="h-4 w-4 rounded accent-emerald-700"> Sematkan di bagian atas</label><button class="rounded-xl bg-[#153d36] px-5 py-3 text-sm font-bold text-white hover:bg-[#205148]">Terbitkan pengumuman</button></div>
                        </form>
        ...
        <form method="POST" action="{{ route('boards.tasks.store', $board) }}" enctype="multipart/form-data" class="p-6 sm:p-7">
            ...
            <div class="mt-7 grid gap-5">
                ...
                <label class="grid gap-2 text-sm font-bold">Judul tugas
                    <input id="task-title" name="title" value="{{ old('title') }}" required maxlength="150" placeholder="Contoh: Buat konsep kampanye" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100">
                </label>
                <label class="grid gap-2 text-sm font-bold">Deskripsi <span class="font-normal text-slate-400">(opsional)</span>
                    <textarea name="description" rows="3" maxlength="2000" placeholder="Tambahkan konteks atau detail..." class="resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:bg-white focus:ring-4 focus:ring-emerald-100">{{ old('description') }}</textarea>
                </label>
                <fieldset class="grid gap-2"><legend class="text-sm font-bold">Penanggung jawab <span class="font-normal text-slate-400">(bisa lebih dari satu)</span></legend><div class="grid max-h-32 gap-2 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-2">@foreach ($teamMembers as $member)<label class="flex items-center gap-2 text-xs font-semibold"><input type="checkbox" name="assignee_ids[]" value="{{ $member->id }}" class="rounded accent-emerald-700">{{ $member->name }}</label>@endforeach</div></fieldset>
                <label class="grid gap-2 text-sm font-bold">Lampiran <span class="font-normal text-slate-400">maks. 5 file, masing-masing 10 MB</span>
                    <input type="file" name="attachments[]" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.jpg,.jpeg,.png,.webp,.zip" class="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-3 text-xs file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-50 file:px-3 file:py-2 file:font-bold file:text-emerald-700">
                </label>
                <div class="grid gap-4 sm:grid-cols-2">
                    <label class="grid gap-2 text-sm font-bold">Prioritas
                        <select name="priority" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100">
                    ...
                    <label class="grid gap-2 text-sm font-bold">Tenggat
                        <input name="due_at" type="date" class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100">
                    </label>
                </div>
            </div>
            <div class="mt-7 flex justify-end gap-3">
                ...
                <button class="rounded-xl bg-[#153d36] px-5 py-2.5 text-sm font-bold text-white hover:bg-[#205148]">Simpan tugas</button>
            </div>
        </form>
    </dialog>
</x-layout>
