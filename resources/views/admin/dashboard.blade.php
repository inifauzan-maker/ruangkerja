<x-layout title="Pusat Superadmin">
    <div class="min-h-dvh bg-[#f5f5f5]">
        ...
        <aside id="admin-sidebar" class="fixed inset-y-0 left-0 z-50 flex w-[260px] -translate-x-full flex-col overflow-hidden bg-[#001529] text-white shadow-2xl transition-[width,transform] duration-300 lg:translate-x-0">
            <div class="flex h-16 shrink-0 items-center border-b border-white/10 px-5">
                <a href="{{ route('admin.dashboard') }}" class="flex min-w-0 items-center gap-3">
                    <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#1677ff] text-sm font-extrabold shadow-lg shadow-blue-950/30">RK</span>
                    <span data-admin-sidebar-label class="min-w-0">
                        <span class="block truncate text-sm font-extrabold tracking-tight">RuangKerja</span>
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-5">
                ...
                <div class="grid gap-1">
                    <a href="#overview" class="flex h-11 items-center gap-3 rounded-lg bg-[#1677ff] px-3 text-sm font-bold text-white shadow-sm">
            ...
            <div class="shrink-0 border-t border-white/10 p-3">
                <div class="flex items-center gap-3 rounded-lg bg-white/5 p-2">
                    <span class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-amber-400 text-xs font-extrabold text-[#001529]">{{ strtoupper(substr(auth()->user()->name, 0, 2)) }}</span>
        ...
        <div id="admin-main-shell" class="min-h-dvh transition-[padding] duration-300 lg:pl-[260px]">
            <header class="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200 bg-white/95 px-4 shadow-sm backdrop-blur sm:px-6">
                <button id="admin-mobile-toggle" type="button" class="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-600 hover:border-blue-300 hover:bg-blue-50 hover:text-[#1677ff] lg:hidden" aria-label="Buka navigasi" aria-expanded="false">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16M4 12h16M4 18h16"/></svg>
                </button>
                <button id="admin-collapse-toggle" type="button" class="hidden h-9 w-9 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-[#1677ff] lg:grid" aria-label="Perkecil sidebar" aria-expanded="true">
                ...
                <div class="ml-auto flex items-center gap-2">
                    <a href="{{ route('dashboard') }}" class="hidden h-9 items-center gap-2 rounded-lg border border-slate-200 px-3 text-xs font-bold text-slate-600 hover:border-blue-300 hover:text-[#1677ff] sm:flex">
                    ...
                    <span class="grid h-9 w-9 place-items-center rounded-full bg-[#1677ff] text-xs font-extrabold text-white">{{ strtoupper(substr(auth()->user()->name, 0, 2)) }}</span>
                </div>
            </header>
            ...
            <main class="mx-auto grid max-w-[1680px] gap-5 p-4 sm:p-6">
                ...
                <section id="overview" class="scroll-mt-20">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                            <p class="text-xs font-bold text-[#1677ff]">RINGKASAN PLATFORM</p>
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
                        @foreach ([
                            ['Pengguna', $stats['users'], 'bg-blue-50 text-[#1677ff]', 'M3 21v-2a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v2 M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8 M17 11a3 3 0 1 0 0-6 M21 21v-2a4 4 0 0 0-3-3.87'],
                        ...
                        <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                            <div>
                                <div class="flex items-center gap-2">
                                    <span class="h-5 w-1 rounded-full bg-[#1677ff]"></span>
                            ...
                            <form method="GET" action="{{ route('admin.dashboard') }}#users" class="grid gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_150px_150px_auto]">
                                <label class="relative sm:col-span-2 xl:col-span-1">
                                    ...
                                    <input name="q" value="{{ $filters['q'] }}" maxlength="100" placeholder="Cari nama atau email" class="h-10 w-full rounded-md border border-slate-300 bg-white pl-9 pr-3 text-sm outline-none placeholder:text-slate-400 focus:border-[#1677ff] focus:ring-2 focus:ring-blue-100">
                                </label>
                                <select name="role" class="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#1677ff] focus:ring-2 focus:ring-blue-100">
                                ...
                                <select name="status" class="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#1677ff] focus:ring-2 focus:ring-blue-100">
                                    <option value="">Semua status</option>
                                    <option value="active" @selected($filters['status'] === 'active')>Aktif</option>
                                    <option value="inactive" @selected($filters['status'] === 'inactive')>Nonaktif</option>
                                </select>
                                <div class="flex gap-2">
                                    <button class="h-10 flex-1 rounded-md bg-[#1677ff] px-4 text-sm font-bold text-white shadow-sm hover:bg-blue-600">Filter</button>
                                    @if ($filters['q'] || $filters['role'] || $filters['status'])
                                        <a href="{{ route('admin.dashboard') }}#users" class="grid h-10 w-10 place-items-center rounded-md border border-slate-300 text-slate-500 hover:border-[#1677ff] hover:text-[#1677ff]" aria-label="Hapus filter">×</a>
                                    @endif
                                </div>
                            </form>
                        </div>
                                    ...
                                    <tr class="group hover:bg-blue-50/30">
                                                ...
                                                <div class="min-w-0">
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        @if (auth()->user()->is($user))
                                                            <span class="rounded bg-blue-50 px-1.5 py-0.5 text-[9px] font-bold text-[#1677ff]">ANDA</span>
                                                        @endif
                                                    </div>
                                        ...
                                        <td class="px-5 py-4">
                                            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="flex min-w-[310px] items-center justify-end gap-2">
                                                @csrf
                                                @method('PATCH')
                                                <select name="global_role" class="h-9 rounded-md border border-slate-300 bg-white px-3 text-xs font-semibold outline-none focus:border-[#1677ff]">
                                                ...
                                                <label class="flex h-9 cursor-pointer items-center gap-2 rounded-md border border-slate-300 px-3 text-xs font-semibold">
                                                    <input type="checkbox" name="is_active" value="1" @checked($user->is_active) class="h-4 w-4 rounded border-slate-300 text-[#1677ff] focus:ring-blue-500">
                                                    Aktif
                                                </label>
                                                <button class="h-9 rounded-md border border-[#1677ff] px-3 text-xs font-bold text-[#1677ff] hover:bg-[#1677ff] hover:text-white">Simpan</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
                        ...
                        @forelse ($users as $user)
                            <article class="grid gap-4 p-4">
                                ...
                                <form method="POST" action="{{ route('admin.users.update', $user) }}" class="grid grid-cols-2 gap-2">
                                    @csrf
                                    @method('PATCH')
                                    <select name="global_role" class="h-10 rounded-md border border-slate-300 bg-white px-3 text-xs font-semibold outline-none focus:border-[#1677ff]">
                                        <option value="user" @selected($user->global_role === 'user')>User</option>
                                        <option value="superadmin" @selected($user->global_role === 'superadmin')>Superadmin</option>
                                    </select>
                                    <input type="hidden" name="is_active" value="0">
                                    ...
                                    <button class="col-span-2 h-10 rounded-md bg-[#1677ff] text-xs font-bold text-white hover:bg-blue-600">Simpan perubahan</button>
                                </form>
                            </article>
                        @empty
                    ...
                    <div class="divide-y divide-slate-100 px-4 sm:px-5">
                        @forelse ($recentAudits as $audit)
                            <article class="flex gap-3 py-4">
                                <span class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-blue-50 text-[#1677ff]">
