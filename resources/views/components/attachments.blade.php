    ...
    <div {{ $attributes->merge(['class' => 'mt-3 flex flex-wrap gap-2']) }}>
        @foreach ($attachments as $attachment)
            <div class="group/file inline-flex max-w-full items-center gap-1 rounded-lg border border-slate-200 bg-white/90 p-1 shadow-sm">
                <a href="{{ route('attachments.preview', $attachment) }}" class="inline-flex min-w-0 items-center gap-2 rounded-md px-2 py-1 text-left hover:bg-emerald-50" title="Preview {{ $attachment->original_name }}">
                ...
                <a href="{{ route('attachments.show', $attachment) }}" class="rounded-md px-1.5 py-1 text-xs text-slate-400 hover:bg-emerald-50 hover:text-emerald-700" title="Download">↓</a>
